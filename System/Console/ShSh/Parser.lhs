...
simple :: P Expression
simple = fmap Simple $ many $ word <|> redirectionOperator <|> ioNum
...
table = [[inf (oper Pipe) (:|:) E.AssocRight]
        ...
        ,[postf (oper Async) RunAsync]
        ,[inf (oper Semi <|> newline) (:>>:) E.AssocLeft,
          inf (oper Async) (\a b->RunAsync a :>>: b) E.AssocLeft]]
    where inf s f assoc = E.Infix (s >> return f) assoc
          postf s f = E.Postfix (s >> return f)

expr :: P Expression
expr = E.buildExpressionParser table compound <?> "expression"
